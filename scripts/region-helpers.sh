...
load_regions() {
    ...
    cat regions.json | jq -r '.supportedRegions[].code' 2>/dev/null || {
        print_error "Failed to parse regions.json or jq not installed"
        return 1
    }
}
...
# Get VPN server IP from compute stack
get_vpn_server_ip() {
    local region=$1
    ...
    echo "$outputs" | jq -r '.[] | select(.OutputKey=="VPNServerIP") | .OutputValue' 2>/dev/null
}
