...
export class OwnvpnComputeStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: OwnvpnComputeStackProps) {
    ...
    userDataScript.addCommands(
      ...
      'apt-get install -y wireguard-tools fail2ban ufw curl qrencode awscli',
      '',
      ...
      '',
      '# Only create default client configuration if no existing config was restored',
      'if [ "$WIREGUARD_CONFIG_EXISTS" = "false" ]; then',
      '    echo "Creating default client configuration..."',
      '    /etc/wireguard/add-client.sh macos-client',
      '    ',
      '    # Backup newly created WireGuard configuration to S3',
      '    echo "Backing up new WireGuard configuration to S3..."',
      '    aws s3 sync /etc/wireguard "s3://$S3_BUCKET/wireguard-config/" --exclude "*.tmp" --region "$AWS_REGION"',
      '    echo "WireGuard configuration backed up to S3 successfully"',
      'else',
      '    echo "Using restored WireGuard configuration from S3"',
      'fi',
      '',
